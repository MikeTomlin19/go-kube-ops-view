...
This derivative remains licensed under the GNU General Public License version
3, matching the original project license. See [LICENSE](LICENSE) and
[NOTICE.md](NOTICE.md).
...
``` {.sourceCode .bash}
$ kubectl proxy &
$ docker build -t go-kube-ops-view:latest .
$ docker run -it --net=host go-kube-ops-view:latest
```
...
$ docker build -t go-kube-ops-view:latest .
$ docker run -it -p 8080:8080 -e CLUSTERS=http://docker.for.mac.localhost:8001 go-kube-ops-view:latest
```
...
``` {.sourceCode .bash}
$ docker build -t go-kube-ops-view:latest .
$ docker run -it -p 8080:8080 go-kube-ops-view:latest --mock
```
...
See [docs/multiple-clusters.rst](docs/multiple-clusters.rst) for details.
