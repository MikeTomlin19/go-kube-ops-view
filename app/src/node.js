...
export class Node extends PIXI.Graphics {
    ...
    getResourceUsage() {
        const resources = {}
        const capacity = this.node.status.capacity || this.node.capacity || {}
        const allocatable = this.node.status.allocatable || this.node.allocatable || {}
        for (const key of Object.keys(capacity)) {
            resources[key] = {
                'capacity': parseResource(capacity[key]),
                'reserved': 0,
                'requested': 0,
                'used': 0
            }
            const allocatableValue = allocatable[key]
            if (allocatableValue) {
                resources[key]['reserved'] = resources[key]['capacity'] - parseResource(allocatableValue)
            }
        }
        if (this.node.usage) {
