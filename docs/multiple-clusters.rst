...
.. code-block:: bash
    ...
    $ docker run -it --net=host -v ~/.kube:/kube go-kube-ops-view --kubeconfig-path=/kube/config
...
.. Note::
    ...
    .. code-block:: bash
        ...
        $ docker run -it --net=host -v ~/.kube:/kube -v ~/.minikube:$HOME/.minikube go-kube-ops-view --kubeconfig-path=/kube/config
...
.. code-block:: bash
    ...
    $ docker run -it --net=host -v ~/.kube:/kube go-kube-ops-view --kubeconfig-path=/kube/config --kubeconfig-contexts=bar
...
.. code-block:: bash
    ...
    $ token=mysecrettoken
    $ docker run -it -p 8080:8080 -e OAUTH2_ACCESS_TOKENS=read-only=$token go-kube-ops-view --cluster-registry-url=https://cluster-registry.example.org
